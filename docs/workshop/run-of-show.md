...
- [ ] **Production**: Render (4 services) and Vercel (2 apps) deploys green on the latest
      ...
      Reset all flags to dark: sign in to trader-ops with an admin email (`ADMIN_EMAILS`) and
      toggle the five flags off in the release panel, or `PUT` each with `-H "authorization: Bearer $ADMIN_JWT"` and `{"enabled":false}`.
...
| 0:15–0:40     | **Launch the fleet**   | Open 6 tabs, paste kickoff prompts from `docs/workshop/kickoff-prompts.md` one by one. Linger on pricing: it starts in plan mode — read the plan aloud, approve it.                                                                                                                                                     | Narrate each session's showcase angle: plan-mode approval, TDD, subagent fan-out, agents-building-agents.                                                                                                                                                      |
