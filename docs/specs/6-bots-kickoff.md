...
- Bots have NO inbox, so NO email/OTP. A bot's first and ONLY auth step is provisioning via POST /accounts ({ name, isBot: true }), authenticated with an admin service token the bot MINTS itself — signToken('bots', { admin: true }) from @arena/service-auth (bots hold SESSION_SECRET), sent as Authorization: Bearer <token> (there is NO x-admin-key) → { token, account }; it reuses that returned account token as its Bearer on every call. POST /bets has no accountId (token-derived), needs an idempotencyKey, and may 409 if the price moved (skip it, don't crash).
