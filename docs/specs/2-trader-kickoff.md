...
- Auth is PRE-BUILT: gate the app with @arena/web-auth and call services via its apiFetch. Reads (exposure, leaderboard, flags, state) are any-logged-in-user; the flag FLIP (PUT /flags/:key) additionally needs the flags admin key sent as x-admin-key — the release switch this app owns. Do NOT build login/JWT yourself.
