...
- Auth is PRE-BUILT and IDENTITY-BASED: the global JWT guard from @arena/service-auth is already wired; your control endpoints (POST /play-next, /run, /reset) are admin-only via the shared AdminGuard (the token's `admin` claim — there is NO x-admin-key). When you call other services (pricing /reprice + /reset, betting /settle + /reset), mint an ADMIN service token: signToken('simulator', { admin: true }). Do NOT reimplement auth.
