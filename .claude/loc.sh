...
#   BY COMMIT DAY  net LOC (added-removed) attributed to each commit date, from git history
#   BY WORKSPACE   current-tree line counts per package
#
...
DAY_ROWS=$(git log --numstat --no-renames --pretty=format:'#C%x09%ad' --date=short 2>/dev/null | awk -F'\t' '
  ...
  $3!="" {
    ...
    n = a - r;
    if (p ~ /__tests__/ || p ~ /\.test\./) t[day]+=n; else pr[day]+=n;
  }
...
printf '  BY COMMIT DAY (net functional + test LOC, from git history)\n'
printf '  %-12s %8s %8s %7s %8s %8s\n' 'DATE' 'COMMITS' 'PRODUCT' 'TEST' 'TOTAL' 'MULT'
printf '  %-12s %8s %8s %7s %8s %8s\n' '------------' '-------' '-------' '----' '-----' '----'
HIST_PROD=0; HIST_TEST=0; HIST_COMMITS=0
while IFS=$'\t' read -r d dp dt dc; do
  [ -z "${d:-}" ] && continue
  dtotal=$((dp + dt))
  dmult=$(awk -v a="$dtotal" -v b="$PER_DAY_BASE" 'BEGIN{ printf (b>0 ? "%.1f" : "0.0"), (b>0 ? a/b : 0) }')
  printf '  %-12s %8s %8s %7s %8s %7sx\n' "$d" "$dc" "$dp" "$dt" "$dtotal" "$dmult"
  HIST_PROD=$((HIST_PROD + dp)); HIST_TEST=$((HIST_TEST + dt)); HIST_COMMITS=$((HIST_COMMITS + dc))
done <<< "$DAY_ROWS"
HIST_TOTAL=$((HIST_PROD + HIST_TEST))
HIST_MULT=$(awk -v a="$HIST_TOTAL" -v b="$((PER_DAY_BASE * DAYS_ACTUAL))" 'BEGIN{ printf (b>0 ? "%.1f" : "0.0"), (b>0 ? a/b : 0) }')
printf '  %-12s %8s %8s %7s %8s %8s\n' '------------' '-------' '-------' '----' '-----' '----'
printf '  %-12s %8s %8s %7s %8s %7sx\n\n' "AGGREGATE" "$HIST_COMMITS" "$HIST_PROD" "$HIST_TEST" "$HIST_TOTAL" "$HIST_MULT"
...
printf '  Functional + test LOC     %8s   (product %s + test %s)\n' "$TOTAL" "$PROD" "$TEST"
